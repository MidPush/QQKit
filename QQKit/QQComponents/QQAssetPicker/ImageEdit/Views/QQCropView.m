...
static const CGFloat kQQCropViewPadding = 14.0f;
static const NSTimeInterval kQQCropTimerDuration = 0.8;
static const CGFloat kQQCropViewMinimumBoxSize = 10.0f;
static const CGFloat kQQMaximumZoomScale = 15.0f;

typedef NS_ENUM(NSInteger, QQCropViewOverlayEdge) {
    QQCropViewOverlayEdgeNone,
    QQCropViewOverlayEdgeTopLeft,
    QQCropViewOverlayEdgeTop,
    QQCropViewOverlayEdgeTopRight,
    QQCropViewOverlayEdgeRight,
    QQCropViewOverlayEdgeBottomRight,
    QQCropViewOverlayEdgeBottom,
    QQCropViewOverlayEdgeBottomLeft,
    QQCropViewOverlayEdgeLeft
};

@interface QQCropView ()<UIScrollViewDelegate, UIGestureRecognizerDelegate>
...
@property (nonatomic, strong) QQCropScrollView *scrollView;         /* The scroll view in charge of panning/zooming the image. */
...
@property (nonatomic, strong, readwrite) QQCropOverlayView *gridOverlayView;   /* A grid view overlaid on top of the foreground image view's container. */
...
@property (nonatomic, assign) QQCropViewOverlayEdge tappedEdge; /* The edge region that the user tapped on, to resize the cropping region */
...
- (void)setup
{
    ...
    self.restoreAngle = 0;
    self.cropAdjustingDelay = kQQCropTimerDuration;
    self.cropViewPadding = kQQCropViewPadding;
    self.maximumZoomScale = kQQMaximumZoomScale;
    ...
    self.scrollView = [[QQCropScrollView alloc] initWithFrame:self.bounds];
    ...
    self.gridOverlayView = [[QQCropOverlayView alloc] initWithFrame:self.foregroundContainerView.frame];
...
- (void)updateCropBoxFrameWithGesturePoint:(CGPoint)point
{
    ...
    switch (self.tappedEdge) {
        case QQCropViewOverlayEdgeLeft:
            ...
            break;
        case QQCropViewOverlayEdgeRight:
            if (self.aspectRatioLockEnabled) {
                aspectHorizontal = YES;
            ...
            break;
        case QQCropViewOverlayEdgeBottom:
            if (self.aspectRatioLockEnabled) {
                aspectVertical = YES;
        ...
        case QQCropViewOverlayEdgeTop:
            ...
            break;
        case QQCropViewOverlayEdgeTopLeft:
            if (self.aspectRatioLockEnabled) {
                xDelta = MAX(xDelta, 0);
                yDelta = MAX(yDelta, 0);
            ...
            break;
        case QQCropViewOverlayEdgeTopRight:
            if (self.aspectRatioLockEnabled) {
                xDelta = MIN(xDelta, 0);
                yDelta = MAX(yDelta, 0);
            ...
            break;
        case QQCropViewOverlayEdgeBottomLeft:
            if (self.aspectRatioLockEnabled) {
                CGPoint distance;
                distance.x = 1.0f - (xDelta / CGRectGetWidth(originFrame));
                distance.y = 1.0f - (-yDelta / CGRectGetHeight(originFrame));
            ...
            break;
        case QQCropViewOverlayEdgeBottomRight:
            if (self.aspectRatioLockEnabled) {
            ...
            break;
        case QQCropViewOverlayEdgeNone: break;
    }
    
    //The absolute max/min size the box may be in the bounds of the crop view
    CGSize minSize = (CGSize){kQQCropViewMinimumBoxSize, kQQCropViewMinimumBoxSize};
    ...
    if (self.aspectRatioLockEnabled && aspectHorizontal) {
        ...
        minSize.width = kQQCropViewMinimumBoxSize * aspectRatio;
    }
        
    if (self.aspectRatioLockEnabled && aspectVertical) {
        maxSize.width = contentFrame.size.height * aspectRatio;
        minSize.height = kQQCropViewMinimumBoxSize / aspectRatio;
    }
...
- (QQCropViewOverlayEdge)cropEdgeForPoint:(CGPoint)point
{
    CGRect frame = self.cropBoxFrame;
    ...
    if (CGRectContainsPoint(bottomLeftRect, point))
        return QQCropViewOverlayEdgeBottomLeft;
...
- (void)setCropBoxFrame:(CGRect)cropBoxFrame
{
    ...
    cropBoxFrame.size.width  = MAX(cropBoxFrame.size.width, kQQCropViewMinimumBoxSize);
    cropBoxFrame.size.height = MAX(cropBoxFrame.size.height, kQQCropViewMinimumBoxSize);
    ...
    if (self.croppingStyle == QQImageCropStyleCircular) {
...
- (void)startEditing
{
    ...
    if ([self.delegate respondsToSelector:@selector(cropViewDidStartEditing:)]) {
        [self.delegate cropViewDidStartEditing:self];
    }
}
